...
const CHANGELOG = [
  { title: "A to-do list on Today", detail: "Under today's jobs, a short \"Worth doing\" list built from what the app already knew: the lead nobody rang back, the regular a fortnight past due with nothing booked, the invoice three weeks late, the quote that went quiet. Every item takes one of three answers - call them now, did it, or not now (tomorrow, three days, next week, a month) - and it remembers, so the same handled item doesn't come back tomorrow. Items that would email a customer have a \"Send it\" button, and that press is the only way such an email goes out." },
  { title: "A building hint on leads", detail: "A unit, level or shop number in the address the person typed now shows as a small \"looks like a unit / worth a look first\" flag, and there's a one-tap map lookup for the building's type and storeys where OpenStreetMap knows it. It never suggests a price and never calls a building simple - the only thing it says is whether this one's worth eyeing off before quoting." },
  { title: "Usage tracking tied to the login", detail: "Screen and tap tracking now records which login did it, so two people using the app produce two readable days instead of one blended average. The timeline under Settings gains a filter for whose day you're reading." },
