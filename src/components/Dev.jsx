...
const CHANGELOG = [
  { title: "Regular by default", detail: "Each kind of job has a usual cycle in Settings - windows every 8 weeks, solar once a year, bond cleans never. After a job of a kind that repeats, for a customer not yet on that cycle, Needs you asks once: make it regular? One tap sets it up with the job's date and price; snooze or dismiss says not this one." },
  { title: "Several regular services per customer; nothing sends itself", detail: "A customer can be on more than one cycle - windows every 8 weeks and solar once a year - each with its own due date and usual price, edited inside the customer. Who's due, Needs you, Schedule's Needs booking and the reminder digest all work per service, and finishing a job moves that service's date forward. The automatic-email switches are gone: every message the app wants to send is previewed on Today and sent only when you tap Send." },
  { title: "Bank details, photos on a job, a month view", detail: "Invoices now say where to pay - account name, BSB, number, and the invoice number as the reference - from Settings -> Getting paid. A small camera on each Today card puts a photo on the job; the job record shows them. Schedule has a Month toggle for finding a free day." },
  { title: "Sign in with the phone, work without signal, send an invoice", detail: "Passkeys (Face ID / fingerprint) as the everyday sign-in, password underneath. The five on-site actions - mark done, move, cancel, note, expense - now save on the phone when there's no signal and send themselves when it's back. Jobs can carry a time. Someone new can be added in three fields from the New job form, and an empty app opens with 'add your first customer'. Tabs are in the address, so the phone's back gesture works. An invoice can be emailed from its preview once the email function is set up. The public page has a service search, the owner's own tagline and area, and a reviews link - all from Settings. Push notifications are scaffolded but not sending." },
