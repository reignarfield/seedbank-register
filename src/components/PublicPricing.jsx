...
export default function PublicPricing({ selected, onToggle, onRequestQuote }) {
  return (
    <div className="space-y-5">
      <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm text-blue-900">
        <strong>${MINIMUM_SERVICE_FEE} minimum service fee.</strong> Prices below are a guide - tick anything you're
        interested in and we'll quote it properly, or{" "}
        {BUSINESS_PHONE ? (
      ...
      {selected.length > 0 && (
        <button
          ...
          className="sticky top-3 z-10 w-full flex items-center justify-between gap-3 bg-blue-600 text-white rounded-xl px-4 py-3 shadow-lg shadow-blue-600/20"
        >
          <span className="text-sm font-medium flex items-center gap-2">
            <FileText size={16} /> {selected.length} selected
          </span>
          <span className="text-sm font-semibold">Get a quote for these →</span>
        </button>
      )}
          ...
          <div className="divide-y divide-slate-100 mt-2">
            {group.items.map((item) => {
              ...
              return (
                <label
                  key={item.name}
                  className={`flex items-center justify-between gap-3 px-4 sm:px-5 py-2.5 cursor-pointer transition-colors ${checked ? "bg-blue-50/60" : "hover:bg-slate-50"}`}
                >
